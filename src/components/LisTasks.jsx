...
auth.onAuthStateChanged(function(user){
 if (user){
  if(user.uid === "VHvLXcIdQfZiPRmpj6S6ic85icv1"){
    setAdmin(true)
  }
  else{
    setAdmin(false)
  ...
  const togglePinTask = async (taskId) => {
    try {
      ...
      const res = await axios.put(`http://localhost:5000/tasks/${taskId}/pin`);
      // backend responds with updated task whether pinned is true/false
